...
The operator portal and internal API remain bound to localhost on host ports 9081 and 9443 for product-local diagnostics. The service also joins the product-neutral `degentlemen-sandbox-ingress` Docker network under the `repoprompt` alias so an independently deployed reverse proxy can publish `/portal*` without exposing either port on the host network.

Provision that ingress network once on the sandbox host; neither product owns it:

```sh
docker network create degentlemen-sandbox-ingress
```
