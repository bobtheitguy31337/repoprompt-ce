...
public enum AgentTranscriptPresentationCore {
    public static func project(_ input: AgentSemanticPresentationTurn) -> AgentPresentationTurnWire {
        ...
        func flushCluster() {
            ...
            let tools = cluster.compactMap { row -> AgentPresentationToolWire? in if case let .tool(_, tool) = row { return tool }; return nil }
            let summary = AgentActivityClusterSummaryWire(
                ...
                narration: cluster.compactMap { row -> String? in
                    switch row { case let .thinking(_, text), let .progress(_, text), let .note(_, text): text; default: nil }
                }.last,
