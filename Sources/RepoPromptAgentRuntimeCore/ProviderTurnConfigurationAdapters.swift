...
public struct ProviderTurnConfigurationInput: Sendable {
    ...
    public init(providerID: ProviderSettingsID, model: ProviderModelDescriptor, effortID: String? = nil, permissionID: String? = nil, toolValues: [String: AgentControlValue] = [:], workflowID: String? = nil) {
...
public struct CompiledProviderTurnConfiguration: Sendable {
    ...
    public init(runtimeKind: ProviderKind, providerRawModelValue: String, executionPolicy: ProviderExecutionPolicy, supportsNativeImages: Bool, normalizedToolValues: [String: AgentControlValue]) {
...
public struct ClaudeCompatibleTurnConfigurationAdapter: ProviderTurnConfigurationAdapter {
    ...
    public func compile(_ input: ProviderTurnConfigurationInput) throws -> CompiledProviderTurnConfiguration {
        ...
        let mode: ProviderExecutionMode = switch permission {
        case "claude.requireApproval": .workspaceWrite
        case "claude.autoApproveEdits", "claude.auto": .workspaceWrite
        case "claude.fullAccess": .fullAccess
        default: throw ServiceAPIError(code: .invalidRequest, message: "Claude-compatible permission is invalid")
        }
        ...
        if let effort { native["provider.reasoningEffort"] = effort }
        return .init(runtimeKind: .claudeCompatible, providerRawModelValue: input.model.providerRawValue, executionPolicy: .init(mode: mode, providerSettings: native), supportsNativeImages: input.model.capabilities.nativeImages, normalizedToolValues: settings)
    }
}
