...
public struct UnavailableProviderManagedAuthenticationDriver: ProviderManagedAuthenticationDriving {
    public init() {}
    public func authFlowDescriptor(providerID _: ProviderSettingsID, forceRefresh _: Bool) async -> ProviderAuthFlowDescriptor? { nil }
    public func authenticationState(providerID _: ProviderSettingsID) async -> ProviderManagedAuthenticationState { .unavailable }
    public func logout(providerID _: ProviderSettingsID) async throws {
        throw ServiceAPIError(code: .capabilityMissing, message: "Managed provider authentication is unavailable")
    }
}
...
public actor CodexDeviceAuthDriver: ProviderAuthFlowDriving, ProviderManagedAuthenticationDriving {
    private struct Flow {
        ...
        let loginID: String
        let expiresAt: Date
        let pending: ProviderAuthTransactionStatus
    }
    ...
    public func start(providerID: ProviderSettingsID, kind: ProviderAuthFlowKind) async throws -> ProviderAuthTransactionStatus {
        ...
        guard await startable(forceRefresh: true) else {
            throw ServiceAPIError(code: .capabilityMissing, message: "Codex device authorization is unavailable")
        }
    ...
    public func poll(flowID: UUID) async throws -> ProviderAuthTransactionStatus {
    ...
    private nonisolated func terminal(flowID: UUID, state: ProviderAuthTransactionState, expiresAt: Date, detail: String) -> ProviderAuthTransactionStatus {
