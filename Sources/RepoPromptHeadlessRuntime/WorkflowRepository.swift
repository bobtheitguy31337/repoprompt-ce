...
public actor WorkflowRepository {
    ...
    public func recover() async throws {
        ...
        _ = try validateSnapshot(try await store.workflowRepositorySnapshot(), canonicalBuiltins: builtins)
    }
    ...
    public func create(
        ...
        guard current.workflows.filter({ $0.source == .custom }).count < Self.maximumCustomWorkflows else {
    ...
    public func clone(
        ...
        let current = try await fencedSnapshot(expectedRevision: request.expectedRevision)
        guard current.workflows.filter({ $0.source == .custom }).count < Self.maximumCustomWorkflows,
