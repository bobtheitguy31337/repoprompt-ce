...
public actor AgentSubmissionCoordinator {
    ...
    private func accept(
        ...
        do {
            ...
            let durablePrepared = try JSONDecoder.serviceDecoder.decode(StoredPreparedIntent.self, from: try JSONEncoder.serviceEncoder.encode(preparedIntent))
            let durableCompiled = try JSONDecoder.serviceDecoder.decode(CompiledProviderTurnInput.self, from: compiledJSON)
            let committing = AgentSubmissionRecord(
                ...
                sessionID: session.sessionID,
                identity: identity,
                preparedJSON: try JSONEncoder.serviceEncoder.encode(preparedIntent),
    ...
    public func recover(now: Date = Date()) async throws -> [AgentSubmissionRecord] {
        let preparing = try await store.agentSubmissions(state: .preparing)
        ...
        return try await store.agentSubmissions(state: .accepted, dispatchState: "pending")
    }
