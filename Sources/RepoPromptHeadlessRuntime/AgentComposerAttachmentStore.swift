...
public actor AgentComposerAttachmentStore {
    public struct Configuration: Hashable, Sendable {
        ...
        public init(
            ...
            acceptedRoot: String,
            maximumItemBytes: Int = 10 * 1_024 * 1_024,
            maximumItemsPerTurn: Int = 4,
            maximumStagedBytesPerActor: Int = 40 * 1_024 * 1_024,
            maximumGlobalStagedBytes: Int = 192 * 1_024 * 1_024,
            unclaimedTTL: TimeInterval = 24 * 60 * 60,
            maximumAcceptedBytesPerSession: Int = 256 * 1_024 * 1_024,
            maximumGlobalAcceptedBytes: Int = 2 * 1_024 * 1_024 * 1_024,
            minimumFreeBytes: Int64 = 1 * 1_024 * 1_024 * 1_024,
            maximumPixelCount: Int = 100_000_000,
            maximumDimension: Int = 32_768
        ) {
    ...
    public init(store: SQLiteServiceStore, configuration: Configuration, files: FileManager = .default) throws {
        self.store = store
    ...
    public func recover(now: Date = Date()) async throws {
        let records = try await store.composerAttachments()
    ...
    public func stage(data: Data, displayName: String, declaredMediaType: String?, actorID: String, projectID: UUID, now: Date = Date()) async throws -> ComposerAttachmentWire {
        ...
        let staged = try await store.composerAttachments(lifecycle: .staged)
    ...
    public func preview(attachmentID: UUID, actorID: String, projectID: UUID, visibleSessionID: UUID? = nil, maximumBytes: Int = 10 * 1_024 * 1_024, now: Date = Date()) async throws -> (ComposerAttachmentWire, Data) {
    ...
    public func prepareAcceptance(attachmentIDs: [UUID], submissionID: UUID, actorID: String, projectID: UUID, sessionID: UUID, turnID: UUID, supportsNativeImages: Bool, now: Date = Date()) async throws -> AgentTurnAttachmentManifest {
        ...
        let accepted = try await store.composerAttachments(lifecycle: .accepted)
        ...
        do {
            for record in records {
                ...
                let handle = try FileHandle(forWritingTo: URL(fileURLWithPath: destination)); try handle.synchronize(); try handle.close()
    ...
    private static func fileExtension(for mediaType: String) -> String {
        switch mediaType { case "image/png": "png"; case "image/jpeg": "jpg"; case "image/webp": "webp"; default: "img" }
    }

    private struct RasterMetadata { let mediaType: String; let width: Int; let height: Int }
    ...
    private static func pngMetadata(_ data: Data) throws -> RasterMetadata {
        ...
        while index + 12 <= data.count {
            ...
            if !sawHeader {
                ...
                width = bigEndian32(data, index + 8); height = bigEndian32(data, index + 12); sawHeader = true
            } else if kind == "IDAT" { sawImageData = true }
            else if kind == "IEND" { guard length == 0 else { throw invalidRaster() }; sawEnd = true; index += 12; break }
            index += 12 + length
        }
