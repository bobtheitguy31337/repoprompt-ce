...
public enum LegacySessionJSONImporter {
    ...
    public static func run(source: URL, store: SQLiteServiceStore, projectRoot: URL? = nil, faultAfterImportedSessions: Int? = nil) async throws -> LegacyImportReport {
        ...
        for file in files {
            ...
            for object in objects {
                ...
                records.append(Record(sourceURL: file, sourceDigest: CanonicalSigning.bodyDigest(canonical), object: object, sessionID: sessionID, projectID: projectID, parentSessionID: uuid(object["parentSessionID"])))
            }
        }
        ...
        for projectID in Set(records.map(\.projectID)).sorted(by: { $0.uuidString < $1.uuidString }) {
            ...
            let digest = CanonicalSigning.bodyDigest(Data("project:\(projectID.uuidString):\(baseRoot.path)".utf8))
    ...
    private static func stableUUID(_ seed: String) -> UUID {
        let digest = CanonicalSigning.bodyDigest(Data(seed.utf8))
