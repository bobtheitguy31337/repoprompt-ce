...
final class RemoteIrohGatewayAdapter {
    ...
    private func gatewayRequest(for rpc: RemoteRPCRequest) throws -> RemoteGatewayRequest {
        ...
        case .transcript:
            ...
            let paging: RemoteTranscriptPageRequest = if let itemID = request.itemID {
                .exactItem(itemID)
            } else if request.before != nil {
                .recentBackward(before: request.before)
            } else {
                .legacyForward(afterSequenceIndex: request.afterSequenceIndex)
            }
