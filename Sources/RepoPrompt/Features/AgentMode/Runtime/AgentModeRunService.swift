...
final class AgentModeRunService {
    struct Dependencies {
        ...
        let beginAuthorityRun: (AgentTabSession) async throws -> RunBindingSnapshot
        let settleAuthorityStartFailure: (AgentTabSession) async -> Void
    ...
    func startRun(
        ...
        // Hosted tests and the macOS coordinator path have no durable dispatcher.
        // Skip the stub `beginAuthorityRun` so Codex/Claude/ACP liveness still
        // reaches the legacy provider runners.
