...
/// Write-only credential input. Servers must never echo, persist in SQLite,
/// place in events, or include this value in error descriptions.
...
/// Transient projection. `userCode` and `verificationURL` may only be returned
/// after matching the authenticated initiating administrator identity.
public struct ProviderAuthTransactionStatus: Codable, Hashable, Sendable {
    ...
    public init(flowID: UUID, providerID: ProviderSettingsID, kind: ProviderAuthFlowKind, state: ProviderAuthTransactionState, userCode: String? = nil, verificationURL: URL? = nil, expiresAt: Date, detail: String? = nil) {
        self.flowID = flowID
        self.providerID = providerID
        self.kind = kind
        self.state = state
