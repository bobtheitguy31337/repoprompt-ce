...
public enum CanonicalSigning {
    ...
    public static func canonicalJSONObject(_ data: Data, removingTopLevelKeys: Set<String> = []) throws -> String {
        ...
        if var object = value as? [String: Any] {
            for key in removingTopLevelKeys {
                object[key] = nil
            }
            value = object
        }
    ...
    private static func renderCanonicalJSON(_ value: Any) throws -> String {
        ...
        case let value as String:
            return try String(decoding: JSONSerialization.data(withJSONObject: [value]), as: UTF8.self).dropFirst().dropLast().description
        ...
        case let value as [String: Any]:
            let entries = try value.keys.sorted().map { key in
                let encodedKey = try String(decoding: JSONSerialization.data(withJSONObject: [key]), as: UTF8.self).dropFirst().dropLast()
                return try "\(encodedKey):\(renderCanonicalJSON(value[key] as Any))"
            }
