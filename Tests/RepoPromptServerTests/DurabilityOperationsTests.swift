...
final class DurabilityOperationsTests: XCTestCase {
    ...
    func testMutationDrainStopsAdmissionAndWaitsForAcceptedMutation() async throws {
        let controller = MutationDrainController()
        let admitted = await controller.beginMutation()
        XCTAssertTrue(admitted)
        let drainTask = Task { await controller.drain(timeout: 1) }
        try await Task.sleep(for: .milliseconds(30))
        let admittedDuringDrain = await controller.beginMutation()
        XCTAssertFalse(admittedDuringDrain)
        await controller.finishMutation()
    ...
    func testMutationDrainHasHardDeadline() async {
        ...
        XCTAssertTrue(admitted)
        let started = Date()
        let snapshot = await controller.drain(timeout: 0.05)
